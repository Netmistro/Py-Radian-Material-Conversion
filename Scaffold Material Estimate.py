...
if "Priced Sheet" in wb.sheetnames:
    print("The pricing sheet already exists, recreating new sheet!")
else:
    ws1 = wb.create_sheet("Priced Sheet")
...
headers = ["Item", "Material Description", "Quantity", "Sale Unit Price", "Sale Price", "Sale Used Price", "Used Price",
           "Rental Unit Price", "Rental Price", "Weight (lbs)"]
i = 1
for item in headers:
...
for p in range(1, 11):
    ...
    boldHeadersText.font = Font(bold=True, name="Tahoma", size=11)
...
for item in range(12, sheet_last_row()):
    ...
    Sheet2.cell(row=x, column=10, value=materialWeight)
